          ...
          if(k2 > 0)
            {
              ...
              /* SuperEasy neutrino linear response */
              double ser_mod_fac = 1.;

              if(All.m_nu1 == 0) { // degenerate hierarchy
                ser_mod_fac = Nulinear.poisson_mod_fac(sqrt(k2), All.Time);
              } else { // non-degenerate hierarchy
                ser_mod_fac = Nulinear.poisson_mod_fac_gen(sqrt(k2), All.Time);
	      }
  ...
  if(ThisTask == 0)
    {
      ...
      fprintf(fd, "%d\n", count_non_zero_bins);
      fprintf(fd, "%g\n", All.BoxSize);
      fprintf(fd, "%g\n", All.Hubble * Nulinear.Hubble_Extern(All.Time)); 
      fprintf(fd, "%d\n", (int)(PMGRID));
